...
- [ ] 5 回連続失敗でアカウントがロックされる
- [ ] 未ログイン状態でログイン画面にリダイレクトされる
...
| # | タスク | 見積もり | 担当 | 状態 |
|---|--------|---------|------|------|
| 1.1 | プロジェクト初期セットアップ（Spring Boot + Gradle + PostgreSQL + Flyway） | 4h | - | [ ] |
| 1.2 | users テーブルマイグレーション作成 | 1h | - | [ ] |
...
| 1.7 | JWT トークン生成・検証・Spring Security 設定 | 4h | - | [ ] |
| 1.8 | REST API 実装（POST /api/v1/auth/login, POST /api/v1/auth/register） | 3h | - | [ ] |
| 1.9 | アカウントロック機能（5 回連続失敗） | 2h | - | [ ] |
| 1.10 | 認証フィルター（未認証リダイレクト、ロール判定） | 2h | - | [ ] |
...
| # | タスク | 見積もり | 担当 | 状態 |
|---|--------|---------|------|------|
| 2.1 | フロントエンドプロジェクト初期セットアップ（React + Vite + TypeScript + Tailwind CSS） | 3h | - | [ ] |
| 2.2 | 認証 Context・API クライアント設定（Axios + JWT 自動付与） | 2h | - | [ ] |
| 2.3 | ログイン画面（S-001）実装 | 3h | - | [ ] |
| 2.4 | 新規登録画面（S-002）実装 | 3h | - | [ ] |
| 2.5 | ルーティング・認証ガード（ProtectedRoute）実装 | 2h | - | [ ] |
| 2.6 | ロール別リダイレクト（得意先→商品一覧、スタッフ→ダッシュボード） | 1h | - | [ ] |
...
| # | タスク | 見積もり | 担当 | 状態 |
|---|--------|---------|------|------|
| 5.1 | バックエンド統合テスト（認証 API + DB） | 3h | - | [ ] |
| 5.2 | バックエンド統合テスト（単品 API + DB） | 2h | - | [ ] |
| 5.3 | フロントエンドコンポーネントテスト | 2h | - | [ ] |
| 5.4 | E2E テスト（ログイン→ダッシュボード遷移） | 2h | - | [ ] |
| 5.5 | Docker Compose 環境整備 | 2h | - | [ ] |
