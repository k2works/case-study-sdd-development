...
- [ ] ログイン・ログアウトが正常に動作する
- [ ] 新規アカウント登録後に自動ログインされる
- [ ] 単品の登録・一覧表示・編集・削除が動作する
- [ ] ヘキサゴナルアーキテクチャの実装パターンが確立される（ArchUnit テストで検証）
- [ ] テストカバレッジ 80% 以上（バックエンド・フロントエンド共通）
...
| # | タスク | 見積もり | 担当 | 状態 |
|---|--------|---------|------|------|
| 0.1 | パッケージ構造・共通例外・レスポンス形式（RFC 7807）の確立 | 2h | - | [ ] |
| 0.2 | ArchUnit テスト初期セットアップ（ヘキサゴナルアーキテクチャのレイヤー依存ルール） | 2h | - | [ ] |
| 0.3 | Flyway マイグレーション基盤セットアップ | 1h | - | [ ] |
| 0.4 | Checkstyle 設定・CI 連携 | 1h | - | [ ] |
...
| # | タスク | 見積もり | 担当 | 状態 |
|---|--------|---------|------|------|
| 1.1 | ドメインモデル TDD（認証ユーザー、ユーザー、ロール） | 3h | - | [ ] |
| 1.2 | アプリケーション層 TDD（認証ユースケース） | 2h | - | [ ] |
| 1.3 | インフラ層実装（Spring Security + JWT 設定、Port 経由でアダプター実装） | 4h | - | [ ] |
| 1.4 | REST API TDD（/api/v1/auth/login, /api/v1/auth/logout） | 2h | - | [ ] |
| 1.5 | フロントエンド ログイン画面コンポーネント TDD（Vitest + React Testing Library） | 3h | - | [ ] |
| 1.6 | フロントエンド 認証状態管理 TDD（Context） | 2h | - | [ ] |
| 1.7 | フロントエンド ルートガード TDD（未認証リダイレクト） | 1h | - | [ ] |
| 1.8 | アカウントロック機能 TDD（5 回連続失敗、30 分ロック、カウントリセット） | 2h | - | [ ] |
| 1.9 | セキュリティテスト（JWT 検証・改ざん検知、認可、パスワードハッシュ検証） | 2h | - | [ ] |
| 1.10 | 統合テスト・E2E テスト | 3h | - | [ ] |
...
| # | タスク | 見積もり | 担当 | 状態 |
|---|--------|---------|------|------|
| 2.1 | ドメインモデル TDD（アカウント登録ロジック、パスワードポリシー） | 2h | - | [ ] |
| 2.2 | アプリケーション層 TDD（登録ユースケース） | 2h | - | [ ] |
| 2.3 | REST API TDD（/api/v1/auth/register） | 1h | - | [ ] |
| 2.4 | フロントエンド 新規登録画面コンポーネント TDD | 2h | - | [ ] |
| 2.5 | 重複メールアドレスチェック TDD | 1h | - | [ ] |
| 2.6 | 登録後の自動ログイン TDD | 1h | - | [ ] |
| 2.7 | 統合テスト | 2h | - | [ ] |
...
| # | タスク | 見積もり | 担当 | 状態 |
|---|--------|---------|------|------|
| 3.1 | ドメインモデル TDD（単品、品質維持日数、リードタイム、購入単位） | 3h | - | [ ] |
| 3.2 | アプリケーション層 TDD（単品 CRUD ユースケース） | 2h | - | [ ] |
| 3.3 | インフラ層 TDD（リポジトリ、DB マイグレーション） | 2h | - | [ ] |
| 3.4 | REST API TDD（/api/v1/items CRUD） | 2h | - | [ ] |
| 3.5 | フロントエンド 単品一覧・登録画面 TDD | 3h | - | [ ] |
| 3.6 | バリデーション TDD（必須項目・境界値チェック） | 1h | - | [ ] |
| 3.7 | 統合テスト | 2h | - | [ ] |
...
|---------|----|----|------|
| プロジェクト基盤整備 | - | 6h | [ ] |
| 認証基盤・ログイン（US-017） | 5 | 24h | [ ] |
| アカウント新規登録（US-018） | 3 | 11h | [ ] |
| 単品登録（US-003） | 3 | 15h | [ ] |
| **合計** | **11** | **56h** | |
