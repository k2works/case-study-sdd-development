...
### UI/UX レビュー指摘の反映（IT1 スコープ）

UI/UX レビュー（2026-03-24）の指摘のうち、IT1 で対応するもの:

| # | 指摘 | 対応方針 | タスク |
| :--- | :--- | :--- | :--- |
| H-01 | 商品一覧・詳細はログイン不要 | 商品 API は認証不要で公開 | 0.4 の URL 設計で反映 |
| H-02 | 空状態（Empty State） | 商品 0 件時のメッセージを定義 | 2.6 の API テストで検証 |
| M-05 | C-04（商品詳細）の情報設計 | 構成花材・価格・画像 URL を含む API レスポンスを設計 | 3.6 で対応 |
| L-17 | カレンダー曜日を日本語表記 | IT2（注文入力画面）で対応 | IT2 に繰り越し |

### 顧客向け画面の認証方針

| エンドポイント | 認証 | 理由 |
| :--- | :--- | :--- |
| GET `/api/products/` | 不要 | 商品閲覧はログイン前でも可能 |
| GET `/api/products/{id}/` | 不要 | 商品詳細もログイン前でも可能 |
| POST `/api/orders/` | 必要 | 注文はログイン必須 |
| GET `/api/orders/` | 必要 | 注文履歴はログイン必須 |
| GET `/api/delivery-addresses/` | 必要 | 届け先はログイン必須 |
