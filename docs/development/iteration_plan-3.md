...
- [x] StockForecastService が日別在庫推移を正しく計算する
- [x] 在庫推移画面で単品ごとの推移を確認できる
- [x] 品質維持日数を超過した在庫が廃棄対象として表示される
- [x] 入荷予定・引当済みが推移に反映される
...
| # | タスク | 見積もり | 状態 |
|---|--------|---------|------|
| 1.1 | Stock モデル（TDD: spec → model → migration） | 4h | [x] |
| 1.2 | PurchaseOrder モデル（TDD） | 3h | [x] |
| 1.3 | Arrival モデル（TDD） | 2h | [x] |
| 1.4 | StockAllocationService（引当テーブル or 論理引当）の方針決定 | 1h | [x] |
...
| # | タスク | 見積もり | 状態 |
|---|--------|---------|------|
| 2.1 | StockForecastService の基本構造（TDD: 依存注入 + 空の forecast メソッド） | 2h | [x] |
| 2.2 | 良品在庫の日別計算ロジック（品質維持日数考慮） | 3h | [x] |
| 2.3 | 入荷予定の計算ロジック（PurchaseOrder ベース） | 2h | [x] |
| 2.4 | 引当済みの計算ロジック（Order の届け日ベース） | 2h | [x] |
| 2.5 | 廃棄対象の計算ロジック（品質維持期限超過） | 1h | [x] |
| 2.6 | 有効在庫（良品 + 入荷予定 - 引当 - 廃棄）の集計 | 1h | [x] |
...
| # | タスク | 見積もり | 状態 |
|---|--------|---------|------|
| 3.1 | StockForecastsController + Request Spec | 3h | [x] |
| 3.2 | 在庫推移一覧画面（単品選択 + 日別テーブル） | 3h | [x] |
| 3.3 | ナビゲーションに「在庫推移」リンク追加 | 0.5h | [x] |
| 3.4 | ルーティング設定 | 0.5h | [x] |
...
| カテゴリ | SP | 理想時間 | 状態 |
|---------|----|----|------|
| データモデル（Stock/PurchaseOrder/Arrival） | 3 | 10h | [x] |
| StockForecastService | 3 | 11h | [x] |
| 在庫推移画面 | 2 | 7h | [x] |
| **合計** | **8** | **28h** | |
