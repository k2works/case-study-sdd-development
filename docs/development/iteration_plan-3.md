...
| # | タスク | 見積もり | 状態 |
|---|--------|---------|------|
| 0.1 | App.tsx をカスタムフックに分割（T1: ルーティング・状態管理・API 呼び出しの分離）+ テスト修正 | 3h | [x] |
| 0.2 | DeliveryDate のバックエンドバリデーション追加（T2: 過去日付の拒否）+ テスト | 0.5h | [x] |
| 0.3 | Item/Product の createNew 型安全化（T5: `undefined as unknown` の解消） | 1h | [x] |
...
| # | タスク | 見積もり | 状態 |
|---|--------|---------|------|
| 3.1 | E2E テスト: 在庫推移画面表示・在庫推移データ表示・発注画面遷移 | 1h | [x] |
| 3.2 | E2E テスト: 発注数量入力→購入単位自動調整→発注確定→在庫推移画面復帰 | 1h | [x] |
| 3.3 | E2E テスト: 在庫推移画面→発注画面遷移→戻るボタンで在庫推移画面復帰 | 1h | [x] |
| 3.4 | 統合テスト・バグ修正（テストサーバーに在庫推移・発注ルート追加） | 1h | [x] |
...
|---------|----|----|------|
| 技術的負債解消・基盤整備 | - | 6h | [x] |
| S08: 在庫推移を確認する | 5 | 12.5h | [x] |
| S09: 単品を発注する | 3 | 9.5h | [x] |
| リリース検証 | - | 4h | [x] |
| **合計** | **8** | **32h** | |
...
- [x] ユニットテストがパス（バックエンド 166 件、フロントエンド 89 件）
- [x] 統合テストがパス（在庫推移計算の正確性、発注→入荷予定反映）
- [x] E2E テスト作成済み（5 シナリオ）※ Playwright ブラウザ未インストール環境のため実行保留
