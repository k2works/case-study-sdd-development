...
- [ ] Rails プロジェクト作成、Devise 認証設定
- [ ] 商品（S01）・単品（S02）の Model/Controller/View
- [ ] 花束構成（S03）の Model/Controller/View

**目標 SP**: 9（S01: 3 + S02: 3 + S03: 3）
...
- [ ] 在庫推移計算（StockForecastService）
- [ ] 在庫推移画面（S08）
- [ ] MVP リリース準備（E2E テスト、デプロイ設定）
...
**目標 SP**: 14（S05: 5 + S06: 3 + S13: 3 + S14: 3）
...
1. IT1 のイテレーション計画を作成する
2. Rails プロジェクトの初期構築を開始する
3. Heroku 環境のセットアップを行う
