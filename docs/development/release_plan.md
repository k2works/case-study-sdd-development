...
### イテレーション 3（Week 5-6）🔄 進行中

**ゴール**: 在庫推移表示の基盤を TDD で実装し、注文キャンセル機能を完成させる

**主なタスク**:

- [ ] IT2 ふりかえり対応（Application 層導入、ファクトリメソッド標準化）
- [ ] US-007: 在庫推移確認（8SP）
- [ ] US-014: 注文キャンセル（3SP）

**目標 SP**: 11

**計画変更**: US-007 を IT3 で全体実装（IT2 では未着手のため）。US-006（届け先コピー）を IT4 に繰り越し。
...
1. イテレーション 3 の開発を開始する
2. 在庫推移（US-007）のドメイン層から TDD で実装
3. IT3 完了後に GitHub Project に同期する
