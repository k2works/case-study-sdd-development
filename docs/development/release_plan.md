...
- [ ] Django プロジェクト初期セットアップ（uv + Django 5.2 + PostgreSQL）
- [ ] US-001: 単品マスタ登録（3SP）
- [ ] US-002: 商品登録（3SP）
- [ ] US-003: 花束構成定義（3SP）
...
**ゴール**: WEB 受注と在庫推移表示の基盤を実装する

**主なタスク**:

- [ ] US-004: 商品選択画面（3SP）
- [ ] US-005: 注文入力・確定（5SP）
- [ ] US-007: 在庫推移確認（8SP の前半 - 在庫推移計算ロジック）

**目標 SP**: 8+α

### イテレーション 3（Week 5-6）

**ゴール**: 在庫推移表示の完成と届け先コピー・キャンセル機能を実装する

**主なタスク**:

- [ ] US-007: 在庫推移確認（8SP の後半 - 画面表示）
- [ ] US-006: 届け先コピー（3SP）
- [ ] US-014: 注文キャンセル（3SP）
...
1. イテレーション 1 の詳細計画を作成（`--iteration 1`）
2. Django プロジェクトの初期セットアップを実行
3. GitHub Project に同期（`syncing-github-project`）
