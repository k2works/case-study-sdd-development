...
| # | タスク | 見積もり | 状態 |
|---|--------|---------|------|
| 1.1 | Shipment モデル作成 + マイグレーション（TDD: order_id FK + UQ, shipped_at） | 2h | [ ] |
| 1.2 | Order モデルに shipping_date, shippable? メソッド追加（TDD） | 1h | [ ] |
| 1.3 | Order has_one :shipment 関連追加 + Order#shipped? ステータス追加 | 1h | [ ] |
...
| # | タスク | 見積もり | 状態 |
|---|--------|---------|------|
| 2.1 | ShippingService#ship（TDD: Shipment 作成 + Order → shipped + Stock 消費） | 3h | [ ] |
| 2.2 | 出荷済み受注の二重出荷防止バリデーション（TDD） | 1h | [ ] |
| 2.3 | 在庫不足時のエラーハンドリング設計（IT4 Try 反映: 最初から網羅的に） | 1h | [ ] |
...
| # | タスク | 見積もり | 状態 |
|---|--------|---------|------|
| 3.1 | ShipmentsController + Request Spec（index: 出荷日フィルタ付き一覧） | 3h | [ ] |
| 3.2 | 出荷一覧画面（出荷日選択 → 対象受注一覧 + 花束構成表示 + チェックボックス選択） | 3h | [ ] |
| 3.3 | 出荷処理アクション（create: 選択した受注を一括出荷） + Request Spec | 2h | [ ] |
| 3.4 | ステータスバッジ helper メソッド化（IT4 Try 反映: 3 画面目なので DRY 適用） | 1h | [ ] |
| 3.5 | ナビゲーションに「出荷管理」リンク追加 | 0.5h | [ ] |
...
| # | タスク | 見積もり | 状態 |
|---|--------|---------|------|
| 4.1 | 発注→入荷→出荷の E2E フロー統合テスト | 2h | [ ] |
| 4.2 | 在庫推移（StockForecastService）との連携確認テスト | 1h | [ ] |
| 4.3 | コードレビュー（developing-review: 2-3 並列で実施、IT4 Try 反映） | 2h | [ ] |
| 4.4 | レビュー指摘対応・バグ修正 | 1h | [ ] |
...
| カテゴリ | SP | 理想時間 | 状態 |
|---------|----|----|------|
| Shipment モデル + Order 拡張 | 1 | 4h | [ ] |
| ShippingService | 2 | 5h | [ ] |
| 出荷一覧画面 + Controller | 3 | 9.5h | [ ] |
| 統合テスト・レビュー | 2 | 6h | [ ] |
| **合計** | **8** | **24.5h** | |
