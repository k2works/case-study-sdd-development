...
| インフラ | RDS FreeableMemory | CloudWatch | 200MB 以下（5 分間） |
...
| バッチ | 失敗時の影響 | リカバリ手順 |
| :--- | :--- | :--- |
| 受注ステータス更新 | 出荷準備が遅延 | 手動で management command を再実行 |
| 品質維持期限チェック | 期限切れ花材の見逃し | 手動再実行 + スタッフへの緊急通知 |
| 廃棄対象チェック | 廃棄対象の未検出 | 手動再実行 |

- 全バッチは冪等性を保証する（再実行しても副作用なし）
- バッチ成功/失敗を CloudWatch メトリクスとして記録
- 失敗時は Slack に即時通知（7:00 に未実行確認通知も送信）
