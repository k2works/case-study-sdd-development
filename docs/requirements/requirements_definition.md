...
| 項目 | 定義 |
| :--- | :--- |
| 基準時刻 | 日付の変わり目（0:00）を基準とする |
| 日数計算 | 届け日を含まない。例: 届け日が 4/10 の場合、4/7 の 23:59 まで変更可能 |
| 出荷日 | 届け日の前日（届け日 4/10 → 出荷日 4/9） |
...
| 出庫予定 | 受注の花束構成を単品レベルに展開（商品構成 × 受注数量）し、出荷日で集計 |
| 入荷予定 | 発注の入荷予定日に基づく数量 |
| 廃棄予定 | 在庫ロットごとに（入荷日 + 品質維持日数）を超過する数量 |

- 表示期間: 当日〜14 日先
- 更新タイミング: 受注登録・入荷登録・キャンセル時にイベント駆動で即時更新
- 品質維持日数の起算: 入荷日を含む（入荷日 = 1 日目）

#### 在庫引当ルール

| 項目 | 定義 |
| :--- | :--- |
| 引当粒度 | 在庫ロットレベル（ロットごとに引当数量を記録） |
| 引当戦略 | 品質維持期限の短いロットから優先的に引当（FIFO） |
| 引当タイミング | 受注確定時に即時引当 |
| 排他制御 | 在庫引当は単品・日付単位で排他制御される |
| 引当解除 | キャンセル時・届け日変更時に解除され、再引当が行われる |
...
| アカウント作成 | 初回注文時に得意先が自ら作成（メールアドレス確認あり） |
| セッション | ログイン後のセッション管理（セッション有効期間は非機能要件で定義） |
| 前提 | 届け先コピー（UC-002）、注文履歴確認（UC-012）はログイン済みが前提 |

#### 花材の代替ルール

| 項目 | 定義 |
| :--- | :--- |
| 代替発生条件 | 発注した花材が入荷不可の場合 |
| 代替判断 | 仕入スタッフが代替花材を選定（人間判断） |
| 受注への影響 | 代替花材で結束する場合、得意先への事前連絡が必要 |
| システム対応 | フェーズ 2 以降で入荷不可時の代替登録機能を検討 |

#### 1 受注 = 1 商品の制約

| 項目 | 定義 |
| :--- | :--- |
| 制約 | 1 回の注文で注文できる商品は 1 つ。1 受注 = 1 届け先 = 1 商品 |
| 将来拡張 | 複数商品の同時注文は将来の検討課題。情報モデルは注文明細の追加が可能な構造とする |
...
frame "注文履歴を確認する" as f04
usecase "注文履歴・注文状況を確認する" as UC5
boundary "注文履歴画面" as b06
boundary "注文詳細画面" as b07
entity "得意先" as e05
