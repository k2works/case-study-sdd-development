...
public class DevDataInitializer implements ApplicationRunner {
    ...
    private record SeedCustomerSpec(
            String email,
            String firstName,
            String lastName,
            String phone) {
    ...
    private static final SeedCustomerSpec CUSTOMER_1 = new SeedCustomerSpec(
            "customer@example.com", "太郎", "山田", "090-1234-5678");

    private static final SeedCustomerSpec CUSTOMER_2 = new SeedCustomerSpec(
            "customer2@example.com", "美咲", "鈴木", "080-9876-5432");
    ...
    public void run(ApplicationArguments args) {
        ...
        ensureCustomerUser(CUSTOMER_1);
        ensureCustomerUser(CUSTOMER_2);
        ...
        // トランザクションデータ作成
        createSeedOrdersFor(CUSTOMER_1, buildCustomer1OrderSpecs());
        createSeedOrdersFor(CUSTOMER_2, buildCustomer2OrderSpecs());
    }

    // --- 得意先1の注文定義: 異なる届け先（本人以外含む） ---
    private List<SeedOrderSpec> buildCustomer1OrderSpecs() {
        LocalDate today = LocalDate.now();
        return List.of(
                new SeedOrderSpec(
                        "赤バラのクラシックブーケ", today.plusDays(3),
                        "山田 花子", "150-0001",
                        "東京都渋谷区神宮前 1-2-3", "090-1111-2222",
                        "お母さんへの誕生日祝いです。", false),
                new SeedOrderSpec(
                        "パステルミックスブーケ", today.plusDays(5),
                        "佐藤 次郎", "220-0004",
                        "神奈川県横浜市西区北幸 2-4-6", "090-3333-4444",
                        "上司の退職祝いに。午前中着でお願いします。", true),
                new SeedOrderSpec(
                        "ホワイトエレガンス", today.plusDays(7),
                        "山田 太郎", "530-0001",
                        "大阪府大阪市北区梅田 3-1-1", "090-1234-5678",
                        null, false)
        );
    }

    // --- 得意先2の注文定義: 異なる届け先 ---
    private List<SeedOrderSpec> buildCustomer2OrderSpecs() {
        LocalDate today = LocalDate.now();
        return List.of(
                new SeedOrderSpec(
                        "パステルミックスブーケ", today.plusDays(4),
                        "鈴木 一郎", "460-0008",
                        "愛知県名古屋市中区栄 3-5-1", "080-1111-2222",
                        "退院祝いです。お元気で。", false),
                new SeedOrderSpec(
                        "感謝のカーネーションブーケ", today.plusDays(6),
                        "鈴木 敏子", "810-0001",
                        "福岡県福岡市中央区天神 1-8-1", "080-5555-6666",
                        "母の日のプレゼントです。", true)
        );
    }
    ...
    private void createStaffUser() {
        ensurePrivilegedUser("staff@example.com", "明美", "受注", null, Role.ORDER_STAFF, "受注スタッフ");
    }
    ...
    private void ensureCustomerUser(SeedCustomerSpec spec) {
        var existing = authUserRepository.findByEmail(spec.email());
        ...
        registrationUseCase.register(
                spec.email(), "Password1",
                spec.firstName(), spec.lastName(), spec.phone());
        log.info("得意先ユーザーを作成しました: {}", spec.email());
    }
    ...
    private void createSeedOrdersFor(SeedCustomerSpec customerSpec, List<SeedOrderSpec> orderSpecs) {
        var customerUser = authUserRepository.findByEmail(customerSpec.email());
        if (customerUser.isEmpty()) {
            log.debug("得意先ユーザーが見つかりません。注文作成をスキップ: {}", customerSpec.email());
            return;
        }

        Long userId = customerUser.get().getId();
        ...
        if (customer.isPresent() && !orderRepository.findByCustomerId(customer.get().getId()).isEmpty()) {
            log.debug("注文が既に存在するためスキップ: {}", customerSpec.email());
            return;
        }

        var products = productRepository.findAll();
        if (products.isEmpty()) {
            log.debug("商品データが存在しないため注文作成をスキップ: {}", customerSpec.email());
            return;
        }

        for (SeedOrderSpec spec : orderSpecs) {
            createSeedOrder(userId, products, spec);
        }

        log.info("開発用受注データを作成しました（{}件）: {}", orderSpecs.size(), customerSpec.email());
    }
    ...
    private void createSeedStocks(List<Item> items, LocalDate today) {
        for (Item item : items) {
    ...
    private void createSeedPurchaseOrders(List<Item> items, LocalDate today) {
    ...
    private void createPurchaseOrderForItem(List<Item> items, String name,
                                             int unitMultiplier, LocalDate today) {
    ...
    private void addCompositionIfItemExists(Product product, List<Item> items, String itemName, int quantity) {
        items.stream()
    ...
    private void createSeedOrder(Long userId, List<Product> products, SeedOrderSpec spec) {
        products.stream()
    ...
    private void ensurePrivilegedUser(String email, String firstName, String lastName,
        ...
        if (existingUser.isEmpty()) {
            ...
            log.info("{}ユーザーを作成しました: {} ({})", label, email, expectedRole);
            return;
        }
